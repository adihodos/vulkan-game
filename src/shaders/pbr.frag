...
layout (std140, set = 1, binding = 0) readonly buffer primitive_pbr_data_t {
	pbr_data_t entry[];
} primitives_pbr_data;

layout (set = 1, binding = 1) uniform sampler2DArray s_colormap;
layout (set = 1, binding = 2) uniform sampler2DArray s_metal_roughness_map;
layout (set = 1, binding = 3) uniform sampler2DArray s_normal_map;

layout (std140, set = 2, binding = 0) uniform pbr_lighting_data_t {
	vec3 eye_pos;
} lighting_data;

layout (set = 3, binding = 0) uniform samplerCube s_irradiance;
layout (set = 3, binding = 1) uniform samplerCube s_prefiltered;
layout (set = 3, binding = 2) uniform sampler2D s_brdf_lut;
...
void main() {
	const pbr_data_t pbr = primitives_pbr_data.entry[fs_in.primitive_id];
